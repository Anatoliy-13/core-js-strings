...
function removeFirstOccurrences(str, value) {
  ...
  if (index > -1) return str.slice(0, index) + str.slice(index + value.length);
  return str;
}
...
function isPalindrome(str) {
  const string = str.toLowerCase().replaceAll(/[^a-z0-9]/g, '');
...
function invertCase(str) {
  return str
    .split('')
    .map((word) => {
      if (word === word.toLowerCase()) return word.toUpperCase();
...
function encodeToRot13(str) {
  const originalAlfa = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const cipher = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(
    /[a-z]/gi,
    (letter) => cipher[originalAlfa.indexOf(letter)]
  );
}
